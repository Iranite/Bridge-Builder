...
public class BridgeBuilder : MachineEntity, PowerConsumerInterface
{
	...
	public int BridgeWidth = 1; // can be 1, 3 or 5 (>1 comes later!)
	public float mrMaxPower;
	public int mnTimeSinceBuild = 0;
	public int bcType = 0;
	public int[] BridgeCubeTypes = { 126 }; // Reinforced Concrete wall
	public int[] GarbageCubes = { 21, 68, 200, 12, 16, 4, 2, 3, 7, 13, 24, 199, 87, 17 };
	public int GarbageAmount = 4;  //1 for Super
	public int GarbageYield = 1; //2 for Super
	public string BuilderName = "Bridge Builder";


	public Vector3 mUp;
	public Vector3 mForwards;
	public Vector3 lMov = Vector3.zero;
	public long buildX;
	public long buildY;
	public long buildZ;
	...
	public override void LowFrequencyUpdate()
	{
		...
		if (CarriedBlock < BridgeWidth)
		{
			...
			if (CarriedBlock == 0) return; //no need to carry on this round...
		}
		...
		{
			//if (mrCurrentPower >= SearchCost) //search is free now.
			
				//mnBuildDelay += 1; //always delay atleast 1 - no, don't. Search fast and free.
				UpdateSearch();
	...
	void UpdateSearch()
	{
		...
		if (CubeHelper.GetCategory(lCube) == MaterialCategories.Decoration) 
		{
			...
			UpdateCost();
			//mrCurrentPower -= SearchCost; //searching costs no power
			Blocked = false;
		}
		...
		else if(mrCurrentPower >= BuildCost && lMov.magnitude < 1)
		{
			// there's no need to build air, the build command will happily swap one voxel type for another, even if there's non air.
			WorldScript.instance.BuildOrientationFromEntity(checkSegment, buildX, buildY, buildZ, (ushort)BridgeCubeTypes[bcType], (ushort)1092, mFlags); // 1092 dark gray
	...
	public void UpdateCost()
	{
		...
		//SearchCost = (float)Math.Sqrt(mnSearchDistance)+FZero; //search is free now.
	...
	public override void UnityUpdate()
	{
		...
		lMov = (lBuilderTarget - Builder.transform.position);
		...
		else if (mrCurrentPower < BuildCost|| CarriedBlock == 0)
		{
	...
	// The intended Format for the UI: x means will always be there! How many lines are possible again? (9)
	//1x	Bridge Builder
	//2x	Power: int/INT - Buildcost: int
	//3x	No valid Hopper! / Needs more Garbage! / All Ready!/ Low Power! /Bridge complete!
	//4x	Waiting for orders... / Build distance is: / was: int
	//5x	(E) to start/reset
	//6x	Build limit: INT
	//7x	(Q): -int, (T): +int - (Shift/Ctrl)
	...
	{	
		...
		lStr += " - Buildcost: " + (BuildCost).ToString("F0");
		...
		else if (mbHasHopper == false)
		{
			lStr += "\nNo valid Hopper!";
		}
		else if (CarriedBlock == 0 && mnTimeSinceBuild > 6)
		{
			lStr += "\nNeeds more Garbage!";
		}
		...
		// lStr += "\n" + lMov.magnitude; Builder speed debug.
		return lStr;
	}
	public override void OnDelete()
	{
		...
		if (WorldScript.mbIsServer == false) return; // I have no clue why this is good.

		if (CarriedBlock > 0) //drop paste
		{
			ItemManager.DropNewCubeStack((ushort)652, TerrainData.mEntries[652].DefaultValue, CarriedBlock*4, mnX, mnY, mnZ, Vector3.zero); 
		}
	}
	...
	public override void Read(System.IO.BinaryReader reader, int entityVersion)
	{
		...
		if (entityVersion < 5) reader.ReadSingle();
		else CarriedBlock = reader.ReadInt32();
